...
#if USE_ACCELERATE
  inline void FuncInplaceGFunc(int n, double* inout) {
    vvtanh(inout, inout, &n);
  }
#else
  inline void FuncInplaceGFunc(int n, double* inout) {
    FuncInplace<GFunc>(n, inout);
  }
#endif
...
#if USE_ACCELERATE
inline void SoftmaxInPlace(int n, double* inout, double* scratch) {
//  if (n <= 0) return;
//  // A limit on the negative range input to exp to guarantee non-zero output.
//  const double kMaxSoftmaxActivation = -86.0f;
//  const double zero = 0;
//
//  double max_output;
//  vDSP_maxvD(inout, 1, &max_output, n);
//
//  double neg_max_output = -max_output;
//  vDSP_vsaddD(inout, 1, &neg_max_output, scratch, 1, n);
//  vDSP_vclipD(scratch, 1, &kMaxSoftmaxActivation, &zero, scratch, 1, n);
//  vvexp(inout, scratch, &n);
//
//  double prob_total = 0.0;
//  vDSP_sveD(inout, 1, &prob_total, n);
//  if (prob_total > 0) {
//    vDSP_vsdivD(inout, 1, &prob_total, inout, 1, n);
//  }
  ...
  double prob_total = 0.0;
  for (int i = 0; i < n; i++) {
    double prob = inout[i] - max_output;
    prob = exp(ClipToRange(prob, -kMaxSoftmaxActivation, static_cast<double>(0)));
    prob_total += prob;
    inout[i] = prob;
  }
  if (prob_total > 0.0) {
...
inline void MultiplyAccumulate(int n, const double* u, const double* v,
                               double* out) {
#if USE_ACCELERATE
  vDSP_vmaD(u, 1, v, 1, out, 1, out, 1, n);
#else
  for (int i = 0; i < n; i++) {
    out[i] += u[i] * v[i];
  }
